          ...
          {ticks.map((n): ReactNode => {
            ...
            return (
              <line
                ...
                x1={
                  center +
                  cosRad * (isHour ? radiusInnerMarkerHour : radiusInnerMarker)
                }
                y1={
                  center +
                  sinRad * (isHour ? radiusInnerMarkerHour : radiusInnerMarker)
                }
        ...
        <g className={`${className}_bevel-hours`}>
          {ticks.map((n): ReactNode => {
            ...
            return isHour ? (
              <g
                key={n}
                transform={`translate(${center + Math.cos(rad) * radiusLabel} ${
                  center + Math.sin(rad) * radiusLabel
                })`}
            ...
            {ticks.map((n): ReactNode => {
              ...
              return isHour ? (
                <g
                  key={n}
                  transform={`translate(${
                    center + Math.cos(rad) * radiusLabel
                  } ${center + Math.sin(rad) * radiusLabel})`}
            ...
            {ticks.map((i): ReactNode => {
              ...
              return isHour ? (
                <g
                  key={n}
                  transform={`translate(${
                    center + Math.cos(rad) * radiusLabelInner
                  } ${center + Math.sin(rad) * radiusLabelInner})`}
